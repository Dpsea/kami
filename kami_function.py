...
def oneway(*blocks: Tuple[Block, ...], showid=False, unfold=True, stretch=2, ga=True) -> str:
        ...
        for i in range(_len):
            ...
            for j in reversed(range(wide[0], wide[0] + wide[1] + 1)):
                if flag and _map[i][j] is ' ':
                    ...
                    _map[i][j] = _map[i][j] + interv
                    flag, interv = True, empty + line
                else:
                    _map[i][j] = _map[i][j] + interv
        return _map
    
    def intersect(sequence: Gene, *, unfold=unfold, output=False):
        links = []
    ...
    if _len > 0:
        ...
        if ga:
            _sequence = evolve(_sequence, evaluate=intersect, size=_len, elite=0.4, population=100, generation=100)
